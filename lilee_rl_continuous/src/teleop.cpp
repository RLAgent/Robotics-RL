...
private:
  ...
  void sendTeleopCmd(float, float);
  void setUserControl();
  ...
  a_scale_(1.0)
{
  ...
  teleopcmdpub_ = nh_.advertise<lilee_rl_continuous::TeleopCmd>("teleop_cmd",1);
...
void LileeTeleop::sendTeleopCmd(float linear_vel, float angular_vel)
{
    lilee_rl_continuous::TeleopCmd msg;
    msg.user_control = userControl;
    msg.action.linear_vel = linear_vel;
    msg.action.angular_vel = angular_vel;
...
void LileeTeleop::keyLoop()
{
  ...
  while (ros::ok())
  {
    ...
    {   
        case KEYCODE_L:
            sendTeleopCmd(0.0, 0.5);
            break;
        case KEYCODE_R:
            sendTeleopCmd(0.0, -0.5);
            break;
        case KEYCODE_U:
            sendTeleopCmd(1.0, 0.0);
            break;
        case KEYCODE_D:
            setUserControl();
            break;
